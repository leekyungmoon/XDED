...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        print('Insert WhitenLayers after the following layers: {}'.format(whiten_layers))
    ...
    def featuremaps(self, x, y=None):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        feat_dict['layer0'] = x.detach()

        x = self.layer1(x)
        feat_dict['layer1'] = x.detach()
        if 'layer1' in self.whiten_layers:
            x = self.whiten(x)

        x = self.layer2(x)
        feat_dict['layer2'] = x.detach()
        if 'layer2' in self.whiten_layers:
            x = self.whiten(x)

        x = self.layer3(x)
        feat_dict['layer3'] = x.detach()
        if 'layer3' in self.whiten_layers:
            x = self.whiten(x)

        x = self.layer4(x)
        feat_dict['layer4'] = x.detach()
...
def resnet50_fc512_whiten12(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
...
@BACKBONE_REGISTRY.register()
def resnet50_fc512_whiten123(num_classes, loss='softmax', pretrained=True, **kwargs):
    model = ResNet(
        num_classes=num_classes,
        loss=loss,
        block=Bottleneck,
